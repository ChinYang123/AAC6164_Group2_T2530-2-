...
def get_report():
    ...
    report_text += f"iv. Uptime:\n    - Total: {uptime_str} | Idle: {idle_str}\n"
    report_text += f"v. Processes:\n    - Total: {total_processes} | Running vs Sleeping: {running_cnt}/{sleeping_cnt}\n"
    report_text += f"    - Top 3 CPU: {', '.join([p.info['name'] for p in top_cpu])}\n"
    report_text += f"    - Top 3 Mem: {', '.join([p.info['name'] for p in top_mem])}\n"


    csv_row = [time_csv, cpu_pct, mem.percent, disk.percent, uptime_str, total_processes, running_cnt]
...
def save_to_csv(data_row, filename="logs/performance_report.csv"):
    ...
    with open(filename, mode='a', newline='') as f:
        ...
        if not file_exists:
            writer.writerow(['timestamp', 'cpu_percent', 'memory_percent', 'disk_percent', 'uptime', 'total_processes', 'running_processes'])
