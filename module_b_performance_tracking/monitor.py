...
def get_report():
    ...
    report_text += f"i. CPU Metrics:\n   - Cpu usage percentage: {cpu_pct}% | Load average (1 min, 5 min, 15 min): {load_1:.2f}, {load_5:.2f}, {load_15:.2f} | Number of running processes: {running_cnt}\n"
    report_text += f"ii. Memory:\n   - Total: {m_total:.2f}GB | Used: {m_used:.2f}GB | Available memory: {m_avail:.2f}GB | Memory usage percentage: {mem.percent}%\n"
    report_text += f"iii. Disk:\n   - Total: {d_total:.2f}GB | Used: {d_used:.2f}GB | Free disk space: {d_free:.2f}GB | Disk usage percentage (root partition): {disk.percent}%\n"
...
if __name__ == "__main__":
    try:
        while True:
            ...
            print(">>> Data saved to logs/performance_report.csv. (Ctrl+C to stop)")
            time.sleep(10)
